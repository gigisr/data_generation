...
function_list_to_remove <- c(
    "calcCumulative", "filterTbl", "level1AdjValues", "level1Values", 
    "level2Values", "level3Values", "numericValues"
)
```
...
incident_cat_1 <- read_csv(
    'data_in/incident_category_1.csv',
    col_types = col_types
) %>% 
    calcCumulative

incident_cat_1_adj <- read_csv(
    'data_in/incident_category_1_adj.csv',
    col_types = col_types
) %>% 
    calcCumulative

incident_cat_2 <- read_csv(
    'data_in/incident_category_2.csv',
    col_types = col_types
) %>% 
    calcCumulative

claim_flag <- read_csv(
    'data_in/claim_flag.csv',
    col_types = col_types
) %>% 
    calcCumulative
```
...
incidents_tbl <- policies_tbl[ 
    ...
    group_by( start_year ) %>% 
    mutate(
        description_level_1 = level1Values( tbl = incident_cat_1, n = n() ),
    ...
    mutate(
        description_level_2 = level2Values(
            odds_tbl     = incident_cat_2,
            filter_value = description_level_1,
            n            = n()
        )
    ) %>% 
    ungroup %>% 
    ...
    mutate(
        claim_flag = level2Values(
            odds_tbl     = claim_flag,
            filter_value = incident_year,
            n            = n()
        ) %>% 
            as.logical
    ) %>% 
